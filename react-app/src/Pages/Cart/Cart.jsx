...
import { cartItems, addProductToCart, removeProductFromCartById, deleteProductFromCartById, updateWalletbyId} from '../../api/authenticationService';
...
function Cart() {
    ...
    const [wall, setWall] = useState(state.userInfo.user.wallet);
    ...
    const fetchList = () =>{
        cartItems(state.userInfo.user.userId).then(res => {
            ...
            res.data.forEach(e => {
                deleteProductFromCartById(e.product.id, state.userInfo.user.userId).then(res => {
                    // alert(res.data);
                    fetchCartItems();
                }).catch(err => {
    ...
    const updateWallet = () => {
        let wallet = state.userInfo.user.wallet;
        console.log(state.userInfo, totalPrice);
        if(wallet<totalPrice){
            
            alert("Your wallet has low balance. Please recharge to continue with the purchase.");
        }
        else{
            let updatedWallet = wallet - totalPrice;
            updateWalletbyId(state.userInfo.user.userId, updatedWallet).then(res=>{
                alert(res.data);
            }).catch(err => {
                console.log("Error-Handle: ", err)
            })
            setWall(updatedWallet);
        }
                    ...
                    (
                        <div className="row">
                            ...
                            <div className="col-md-4 position-static top-10 bottom-0 ">
                                <h1>Checkout</h1>
                                <h5>Wallet balance:  <span>{wall}</span></h5>
                                <h5>Total price:  <span>{totalPrice}</span></h5>
                                <p className='text-muted'>Delivery Address</p>
                                <button className='btn btn-warning'  onClick={function(event){ updateWallet(); fetchList()}}>checkout</button>
                                {/* onClick={() => {updateWallet(); fetchList();}} */}
                            </div>
                        </div>
                    )
            }
